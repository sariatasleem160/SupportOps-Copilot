"""Shared OpenAI client with JSON parsing, retry, and cost tracking."""
...
# Pricing per 1M tokens (USD) for gpt-4o-mini — update if model changes.
    ...
    if few_shot:
        for example in few_shot:
            messages.append(
                {
                    "role": "user",
                    "content": example["customer_message"],
                }
            )
            messages.append(
                {
...
def call_llm(
    ...
    *,
    ...
    """Call OpenAI chat completions and return raw text plus usage stats."""
    ...
    messages = _build_messages(system_prompt, user_content, few_shot)
